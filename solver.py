...
def to_rpn(str):
    ...
    while i < len(str):
        token = str[i]
        if token.isalpha() or token.isnumber():
...
def solve_expression(left_side, right_side, dic_vars):
    left_side = get_tokens(left_side)
    rpn_part = to_rpn(left_side)
    print(rpn_part)
