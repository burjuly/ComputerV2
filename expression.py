...
def parse_expression_with_functions(left_side, right_side, right_side_type, dic_vars):
    ...
    while i < len(left_side):
        ...
        if re.search(r'^([a-zA-Z])([\(])([-]?)([0-9]+)([\)])', left_side[i:]):
            ...
            dic_vars = functions.function_of_num(left_side, right_side, right_side_type, dic_vars, False)
            i += len(func_num)
            ...
            need_to_print = False
            dic_vars = functions.function_of_var(func_var, right_side, right_side_type, dic_vars, need_to_print)
            i += len(func_var)
            tmp.append(dic_vars['i'])
            dic_vars = parsing.del_tmp_vars(dic_vars)
        else:
